...
- **`pi/`** — pi-specific artifacts. Single-file TypeScript extensions at this dir's root (e.g., `pi/ai-permission-gate.ts`); test files alongside (e.g., `pi/ai-permission-gate.test.mjs`); `agents/` for subagent templates. All symlinked into `~/.pi/agent/` locations per `README.md` installation.
