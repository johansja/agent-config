...
5. **Deployment health** — broken symlinks: `find ~/.config/opencode ~/.claude ~/.pi/agent ~/.agents -type l ! -exec test -e {} \; -print`. Also: in-repo-but-not-deployed and deployed-but-not-in-repo files across extensions, agents, skills, commands. Every entry in a deployed dir (prompts, agents, skills, commands, bin) must be a symlink resolving inside this repo — flag real files and foreign targets (e.g. symlinks into `$(npm root -g)` package examples) as unmanaged drift. Exclusions: vendor-managed files — see README § Extensions (pi) and § Skills (shared).
