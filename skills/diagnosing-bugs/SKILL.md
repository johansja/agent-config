...
Redact before showing anything: this skill has you paste commands, outputs, and captured artifacts — **redact every secret first** (write `<REDACTED>`), build loops against env vars so credentials stay in the environment rather than in what you show, and quote only the signal-carrying lines of a captured artifact (it carries auth headers). If the redacted output isn't enough to diagnose, say so and ask the user.
