...
description: Audit durable doc artifacts (plans, ADRs, design docs, READMEs, architecture docs) for session-cruft before finalize or merge. Use when writing, revising, or reviewing such docs — especially after a grill/review round or a design pivot. Cuts process provenance; keeps the durable why.
---
...
**Pre-write self-check** (before you call a doc edit done): re-read your diff. For each date, session-ref, or pointer phrase, ask "would a reader six months out understand this without the session?" If no → rewrite as the durable why and drop the marker.

**Pre-merge audit** (also exposed as `/cruft-review`): run all three greps → list hits with verdicts (cut / keep / judgment-call) → fix the cuts → re-grep to confirm zero cruft in durable sections.
